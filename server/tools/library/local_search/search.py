...
passing the ``id`` verbatim. Two kinds of files are supported side by side:
  ...
  * Office documents (``.docx``, ``.xlsx``, ``.pptx``) — the OOXML is unzipped
    and flattened to text with the stdlib (see ``office_text``), then scored
    exactly like a text file, so their ids are ``f:<relpath>:<line>`` too. A
    presentation gets a ``## Slide N`` heading per slide and a workbook a
    ``## Sheet: name`` heading per sheet, which is what a hit is cited by.
...
# Everything read_text_file can turn into text — the one list collect_files,
# chunks_for and main's single-file branch all test against, so a format cannot
# be searchable in one of them and invisible in another.
READABLE_EXTS = TEXT_EXTS | OFFICE_EXTS
...
def collect_files(root):
    ...
    Office documents ride in the TEXT list: everything downstream reaches their
    content through read_text_file and cites them with the same
    ``f:<relpath>:<line>`` ids, so they need no branch of their own.
    """
    text, pdfs = [], []
    for dirpath, dirnames, filenames in _walk(root):
        for name in sorted(filenames):
            ext = os.path.splitext(name)[1].lower()
            if ext in READABLE_EXTS and len(text) < MAX_TEXT_FILES:
...
def main():
    ...
    if os.path.isfile(root):
        ...
        text_files = [root] if ext in READABLE_EXTS else []
    ...
    if not zim_files and not text_files and not pdf_files:
        print(f"No searchable files in {root} (supported: *.zim, *.pdf, "
              f"{', '.join(sorted(TEXT_EXTS | OFFICE_EXTS))}).")
        return
